...
# When jobs are repo-scoped, ~/.lintel/jobs/_active.md holds one line per job
# across ALL repos. Each line carries an HTML-comment marker with the owning
# repo path; syncing = drop this repo's lines, re-append the current ones.
_registry_sync() {
  [ "$_JOBS_SCOPE" = "repo" ] || return 0
  local reg="$LINTEL_JOBS_REGISTRY"
  mkdir -p "$(dirname "$reg")" 2>/dev/null || true

  local marker="<!-- repo:$_JOBS_REPO_ROOT -->"
  local tmp="$reg.tmp"
  {
    ...
    for d in "$LINTEL_JOBS_DIR"/*/; do
      ...
      printf -- '- `%s` · %s · %s · step:%s · %s %s\n' \
        "$id" "$workflow" "$status" "$current_step" "$_JOBS_REPO_ROOT" "$marker"
    done
  } > "$tmp"
