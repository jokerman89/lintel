...
These run via `/compliance-gate` skill (operator-triggered). Surface checklist; operator confirms each item with judgment.
...
**Skill to run:** `/entra-agent-id-prep`
...
**Skill to run:** `/sensitive-use-report`
...
**Skill to run:** `/transparency-doc-gen`

**Format:** MS Transparency Note template

## How `/compliance-gate` orchestrates these
...
Operator confirms the aggregate. Audit-logged to `~/.jstack/audit/compliance-gate.jsonl`.

## When to run /compliance-gate
...
- `/compliance-gate` skill — orchestrates this checklist
- `MSComplianceAuditor` agent — automated audit pass
