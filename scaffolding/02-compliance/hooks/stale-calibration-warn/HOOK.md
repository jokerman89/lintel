...
- Reads `TRAILBLAZER-CALIBRATION.md` last-updated timestamp (frontmatter or file mtime fallback)
- Compares to current time
- If > 30 days: WARN

## Why warn-only

Operator may have a deadline and need to ship with stale calibration. Don't block, but make the staleness visible. The downstream voice-check verdict carries the UNCALIBRATED-or-STALE stamp, which downstream `/ship` reads.

## Override path

Skills that respect this warning (`/customer-voice-check`, `/msvoice-rewrite`, `/demo-deliverable-gen`) accept `--ignore-stale-calibration` flag that surfaces a reason field. Reason logged to audit.
