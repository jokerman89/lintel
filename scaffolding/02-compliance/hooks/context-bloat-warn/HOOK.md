...
Surfaces context-bloat warning at the 50k token / 80 tool-call soft threshold (configurable). Hooks the periodic check so the operator doesn't have to manually run `/context-tokenwatch`.
...
Skipped — would generate too much noise. The `/context-tokenwatch` skill writes explicit audit events when operator invokes it.
