...
`/tier-stamp-agents` skill scans agents in scope, reads upstream license (or operator-authored MIT default), stamps frontmatter:
...
- **/ship skill** reads tier-stamp before bundling a release. If any restricted-tier agent in scope: refuses or moves to install-only mode.
- **`MSComplianceAuditor` agent** flags any unstamped or mismatched agents.
- **CI in scaffold-mvp template** runs `/tier-stamp-agents --strict` on PRs touching agent directories.

## What happens on tier mismatch

If `/ship` is preparing to bundle a release into an MS-internal MIT repo and finds a restricted-tier agent:
...
- `/tier-stamp-agents` skill
- `MSComplianceAuditor` agent
- `~/.jstack/upstream-sources.yaml` — the 8 SHA-pinned upstreams + their license tiers
- `/ship` skill — refuses restricted-tier bundles into permissive repos
