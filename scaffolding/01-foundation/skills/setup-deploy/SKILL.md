---
name: jstack-setup-deploy
description: Configure deploy targets for /land-and-deploy — write targets, validate, register.
color: orange
...
One-time configuration skill for `/land-and-deploy`. Writes `~/.jstack/deploy-targets.yaml` declaring the named targets (staging, prod, canary, etc.) along with their trigger mechanism (GitHub Actions, Azure DevOps, custom script, manual link).
...
- You want to actually run a deploy — use `/land-and-deploy` (this skill just configures)
...
3. **Per-target interview.** For each target being added/updated, ask via AskUserQuestion:
   ...
   - Health-check endpoint (for `/canary` integration)
...
Ready: /land-and-deploy --target staging | --target prod
```
...
- The actual deploy trigger is `/land-and-deploy`'s job, not this skill's. This skill only writes config.
...
- `/land-and-deploy` — uses the config written here
- `/canary` — uses health_check_url from per-target config
