...
- Pre-`/ship` regression check on perf-sensitive code paths
...
1. **Detect benchmark mechanism.** Check `package.json` for `bench` script, `pyproject.toml` for hyperfine/pytest-benchmark, repo-specific `bench/` directory. If none: ask operator to declare via `~/.jstack/benchmark.yaml`.
...
## Recommendation
Address P1 before /ship. P2 acceptable if intentional — record reason in commit message.
```
...
3 regressions (1 P1, 1 P2). Address P1 before /ship.
```

**Single scenario, deeper sampling:**
```
> /benchmark --scenario cold-start --iterations 20
...
- `PerformanceAnalyzer` subagent — for finding the bottleneck once `/benchmark` flagged a regression
- `/qa` — correctness; `/benchmark` is performance
- `/investigate` — for hypothesis-driven follow-up on a regression
- `/ship` — reads benchmark results as advisory signal (regressions don't block by default; operator can opt-in)
