...
Reverses `/freeze`. Removes paths from the session freeze metadata file, allowing other JStack skills to write to them again. Logged to audit.
...
2. **Read session freeze file.** `~/.jstack/freeze/<session-id>.yaml`.
...
5. **Audit log.** Append to `~/.jstack/audit/freeze.jsonl` with operation: unfreeze.
...
**Remove one:**
```
> /unfreeze src/components/landing/ --reason "ready to land nav change"
✓ Unfrozen. 1 path remains frozen.
```

**Clear all:**
```
> /unfreeze --all --reason "session pivot, opening everything"
✓ All freezes cleared this session.
```

**Path wasn't frozen:**
```
> /unfreeze src/lib/dlxClient.ts
⚠ Not in freeze list. No-op.
```

## See also

- `/freeze` — add to session freeze
