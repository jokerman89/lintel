---
name: jstack-land-and-deploy
description: /ship + deploy. Adds post-merge deploy trigger. Requires explicit per-call auth.
color: red
...
Extension of `/ship` that also triggers a deploy after PR merges. Higher-stakes — production-adjacent. Per JStack Layer 2 compliance: deploy-pipeline triggers REQUIRE explicit per-call authorization, even in auto-mode.
...
- No deploy pipeline configured for the repo — use `/ship` only
- Deploy needs human approval gate even after merge — use `/ship`, trigger deploy manually
...
1. **Run /ship** — full ship gauntlet (review check, sanity, squash, push, PR).
...
3. **Authorization check** — per Layer 2: deploy is per-call-auth. Even though `/land-and-deploy` was invoked with intent, surface "About to trigger `<target>` deploy. Confirm?" Operator confirms explicitly.
...
- Deploy is a Layer 2 production-mutation. Per-call auth is **mandatory** — `/land-and-deploy` invocation alone is not auth. The Step 3 explicit confirmation IS the auth.
...
**Staging deploy:**
```
> /land-and-deploy --target staging
[ship runs, PR merged]
...
```
> /land-and-deploy --target prod --canary 10
[ship runs, PR merged]
...
- `/ship` — ship without deploy
- `/setup-deploy` — configure deploy targets in `~/.jstack/deploy-targets.yaml`
