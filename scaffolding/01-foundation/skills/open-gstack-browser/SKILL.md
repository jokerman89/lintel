---
name: jstack-open-gstack-browser
...
Name preserves continuity with the gstack `/open-gstack-browser` convention even though we're in JStack — the alternative (`/open-jstack-browser`) is less recognizable, and operators carry muscle memory across both stacks.
...
**Open to a URL:**
```
> /open-gstack-browser --url http://localhost:5173
✓ Launched to http://localhost:5173, PID 47823.
```

**Diagnose install:**
```
> /open-gstack-browser --check
