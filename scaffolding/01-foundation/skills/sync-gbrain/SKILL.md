...
Idempotent — re-running is safe. If gbrain isn't set up: this skill suggests `/setup-gbrain` and exits without errors.
...
- gbrain not installed/configured — run `/setup-gbrain` first
...
1. **Preflight.** Check `gbrain` binary + `~/.gbrain/config.json` + worktree `.gbrain-source` pin. If any missing: surface what's missing + recommend `/setup-gbrain`.
...
✓ Test query ("trailblazer corpus"): returns updated TRAILBLAZER-CORPUS.md
```
...
- **gbrain not set up:** surface `/setup-gbrain` recommendation. Exit cleanly (not an error).
...
Run /setup-gbrain to initialize.
```

## See also

- `/setup-gbrain` — one-time configuration
