...
Session-scoped lockdown of files or directories. While a path is frozen, other JStack skills refuse to modify it. The freeze is metadata in `~/.jstack/freeze/<session-id>.yaml` — not a filesystem lock — so an explicit override is possible if the operator means it.
...
3. **Write to session freeze file.** `~/.jstack/freeze/<session-id>.yaml`:
   ```yaml
   frozen:
...
5. **Audit log.** Append to `~/.jstack/audit/freeze.jsonl`.
...
1. Read `~/.jstack/freeze/<session-id>.yaml` before any Edit/Write.
...
**Lock a directory:**
```
> /freeze src/components/landing/ --reason "working on portal"
✓ Frozen. Other skills will refuse to edit landing/ this session.
```

**Lock multiple with auto-expire:**
```
> /freeze supabase/migrations/ scripts/deploy/ --reason "migration cooldown" --until 1h
...
- `/unfreeze` — remove a path from session freeze
