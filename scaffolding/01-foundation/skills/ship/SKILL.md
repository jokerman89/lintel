...
- Tests fail — fix first, then `/ship`
...
- **Squash conflicts:** report files in conflict, ask operator to resolve manually, then re-run `/ship`.
...
✗ Sanity blocked: secret pattern detected in src/config.ts:42
  ...
  Action: remove the secret, commit fix, re-run /ship
```
...
- `/plan-eng-review` — required gate before /ship
- `/land-and-deploy` — extends /ship with deploy step
