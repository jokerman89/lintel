...
Distinct from `/land-and-deploy --canary`: that one is a one-shot deploy at a fixed percentage. This skill is the multi-step ramp lifecycle.
...
- A `/land-and-deploy --canary` landed at 10%; now you want to ramp
- Suspected risk — gradual expansion is safer than instant 100%
- Rollback dry-run — start at 100%, ramp DOWN to test recovery path

## When NOT to use

- Feature isn't flagged in code — `/canary` operates on flags, not infrastructure
- Single-tenant deploy with no traffic split capability — use `/land-and-deploy` directly
...
- Optional `--health-check <cmd>` — command to verify health between steps (default: read from `~/.jstack/canary.yaml`)
...
Initial state: 10% (set by /land-and-deploy at 14:22)
Target: 100%
Ramp plan: 10% → 50% → 100%
...
- **Flag system not detected:** report + ask operator to configure `~/.jstack/canary.yaml`. Do not proceed.
...
- **Soak interrupted (machine sleep, ctrl-c):** report partial state. Re-running `/canary` from current step is safe (idempotent — reads current flag value).

## Examples

**Standard ramp to 100%:**
```
> /canary --flag feature-new-billing --target 100
...
```
> /canary --flag feature-new-billing --target 0 --steps 100,50,10,0
...
✗ Stopped at 50%. Flag stable. Re-run /canary to continue when ready.
```
...
- `/land-and-deploy --canary <pct>` — initial deploy with canary
