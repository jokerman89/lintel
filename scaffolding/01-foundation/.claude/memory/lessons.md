...
IDs are allocated as one more than the highest existing ID (L-001 for an empty file), compared numerically and never reused. New lessons are appended at the end, oldest first. Record them with `/li:lessons-add` (`bin/li-lessons.py`) so allocation and the conditional write stay mechanical.
