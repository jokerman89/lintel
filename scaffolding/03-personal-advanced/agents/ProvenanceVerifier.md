...
- 1 record: audit log gap (claimed /customer-voice-check at 14:22, no log entry at that time) ⚠
...
[ANOMALY-1] PROV-3f4d2a8b — audit gap
   ...
   Claimed: /customer-voice-check at 2026-05-15T14:22:00Z, verdict PASS
   Audit log: no entry at that time in ~/.jstack/audit/customer-voice-check.jsonl
   Possible causes:
   ...
   Recommendation: re-run /customer-voice-check now to re-establish provenance with current calibration
...
- **Provenance store unreadable / missing:** report + recommend `/setup-gbrain` or manual setup if it's a fresh install.
