...
5. **Cross-link follow-up skills.** Item 4 → `/sensitive-use-report`, Item 5 → `/dsb-prep`, etc.
6. **Aggregate scorecard.**

## Report format

```
MSComplianceAuditor: <scope>
...
|-------------------------------|---------|----------|-----------|
| AGT / agent governance        | yes     | DRAFT    | /entra-agent-id-prep |
...
| Sensitive-use case            | yes     | NOT_RUN  | /sensitive-use-report |
| DSB                           | no      | N/A      | - |
| DPIA                          | yes     | DRAFT    | /dpia-prep --update |
| Transparency note             | yes     | MISSING  | /transparency-doc-gen |
...
## Overall verdict
NEEDS WORK before /ship. 1 P2 + 3 missing on-demand items.
