---
name: jstack-scaffold-customer-demo
   ...
   <name>/
     ...
     demo/
       ...
       recording/
         ...
         takes/.gitkeep
     deliverables/
       handout-DRAFT.md          # customer takeaway (gated by /customer-voice-check)
...
3. **Template-specific seeding.** Based on `--template`:
   ...
   - `ai-feature`: pre-wires /onerai-prep target + /rai-impact-assessment cross-ref
   - `governance`: pre-wires /compliance-gate full-checklist call
...
5. **Pre-wire voice gates.** Mark `demo/script.md` + all `*-DRAFT.md` files with frontmatter `voice: trailblazer, status: requires-customer-voice-check`. Add to `.gitignore` an entry preventing `*-FINAL.md` artifacts without a passed `/customer-voice-check`.
...
- Voice pre-wiring: 3 DRAFT files gated for /customer-voice-check
...
3. Run /customer-voice-check on script when ready
4. Edit deliverables/handout-DRAFT.md (trailblazer-voiced)
5. /compliance-gate before sharing externally
...
**Data-flow demo:**
```
> /scaffold-customer-demo --name azure-arc-hybrid-demo --template data-flow
✓ Scaffolded. Next: fill sample-data/seed.sql + /customer-voice-check the script.
```

**AI-feature demo with audience:**
```
> /scaffold-customer-demo --name copilot-for-legal --template ai-feature --target-audience "legal-tech CIOs"
✓ Scaffolded with /onerai-prep + /rai-impact-assessment cross-refs pre-wired.
```
...
- `/customer-voice-check` — gates the trailblazer-voice DRAFT files
