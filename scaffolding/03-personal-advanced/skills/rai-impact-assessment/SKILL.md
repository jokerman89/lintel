---
name: jstack-rai-impact-assessment
...
Distinct from `/sensitive-use-report` (category classification) and `/onerai-prep` (One RAI submission draft). Impact assessment is the BROADER design-time RAI lens.
...
- Optional `--sensitive-use-report <path>` — output from `/sensitive-use-report`
...
2. **Per RAI principle, structured assessment:**
   ...
   **Privacy & Security:**
   ...
   - Cross-link to `/dpia-prep` output if available
   ...
   **Transparency:**
   ...
   - Disclosure mechanism (link to `/transparency-doc-gen` output if available)
...
- DPIA: prep'd via /dpia-prep, HIGH-RISK triggered (Article 35), Privacy consultation scheduled
...
- Capabilities + limitations: drafted in /transparency-doc-gen output (DRAFT)
- Provenance: each output cites which legal sources informed it
- Disclosure: pending /customer-voice-check on disclosure copy
...
3. Final transparency disclosure wording — needs /customer-voice-check
```

## Compliance integration

- Implements design-time RAI review. Operator can run BEFORE `/sensitive-use-report` or AFTER (best to iterate).
...
- Output cross-linked by `/onerai-prep` for One RAI submission.
...
```
> /rai-impact-assessment --feature case-analysis-ai --source docs/design/case-analysis.md --depth thorough
...
**Quick post-incident:**
```
> /rai-impact-assessment --feature billing-prediction --depth quick
...
- `/sensitive-use-report` — category-specific triage (complementary)
- `/onerai-prep` — consumes this output
- `/dpia-prep` — Privacy & Security principle cross-link
- `/transparency-doc-gen` — Transparency principle cross-link
