...
- Optional `--rai-assessment <path>` — output from `/rais-impact-assessment` (seeds rubric dimensions)
- Optional `--sensitive-use-report <path>` — output from `/rais-sensitive-use` (seeds adversarial-set categories)
...
- Cross-references seeded from `/rais-impact-assessment` and `/rais-sensitive-use`.
...
```
> /cloudtest-eval-suite --feature case-analysis-ai --rai-assessment compliance/rai-impact-DRAFT.md --sensitive-use-report compliance/sensitive-use-DRAFT.md
...
**Baseline-only:**
```
> /cloudtest-eval-suite --feature smart-reply
[3-dimension baseline rubric, generic stubs]
✓ Suite generated. Recommend running /rais-impact-assessment to enrich.
```

## See also

- `/rais-impact-assessment` — feeds dimension scaffolding
