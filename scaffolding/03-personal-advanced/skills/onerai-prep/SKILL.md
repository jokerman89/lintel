...
- AI feature that touches a sensitive-use category (per `/sensitive-use-report`)
...
- Optional `--sensitive-use-report <path>` — output from `/sensitive-use-report` (auto-attached if recent)
...
3. **Build draft sections:**
   ...
   - **Sensitive-use assessment** (from `/sensitive-use-report` if available)
...
Sections: 10
Sensitive-use cross-ref: present (DLP-002, from /sensitive-use-report)
...
3. Update with reviewer feedback via /onerai-prep --update PROV-<id>
...
- Cross-links to `/sensitive-use-report` output for risk-register seeding.
...
- **Sensitive-use report absent + feature is customer-facing AI:** WARN — run `/sensitive-use-report` first; reviewer will require it.
...
**New feature:**
```
> /onerai-prep --feature case-analysis-ai --source docs/design/case-analysis-ai.md
[Intake interview]
...
```
> /onerai-prep --feature legal-summary --sensitive-use-report ~/.jstack/rai/legal-summary-sensitive-use.md
...
**Update post-review:**
```
> /onerai-prep --feature case-analysis-ai --update
...
- `/sensitive-use-report` — feeds risk register
- `/dpia-prep` — if personal data; complementary
- `/transparency-doc-gen` — customer-facing disclosure document
- `/compliance-gate` Item 4 — runs this for sensitive-use category features
