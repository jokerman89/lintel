---
name: jstack-sensitive-use-report
...
# /sensitive-use-report

Evaluates an AI feature against the MS Responsible AI Sensitive Uses categories — uses that warrant additional review because of the consequence of error. Output is a structured report consumed by `/onerai-prep` and the Sensitive Uses review process.
...
- Pre-`/onerai-prep` — feeds risk register
...
6. **Cross-link.** If recommendation ≠ None: emit cross-reference for `/onerai-prep` to consume.
...
## Next steps
1. Run /onerai-prep --feature case-analysis-ai --sensitive-use-report <this file>
...
- Output cross-linked by `/onerai-prep` for risk register seeding.
...
**AI feature triage:**
```
> /sensitive-use-report --feature case-analysis-ai --source src/lib/case-analysis.ts
[Intake interview + source scan]
✓ 4 categories triggered. Sensitive-Uses review REQUIRED.
  Next: /onerai-prep --sensitive-use-report <this output>.
```

**Internal tool clear:**
```
> /sensitive-use-report --feature engineering-velocity-dashboard
[All NO]
...
- `/compliance-gate` Item 4 — surfaces Sensitive-Uses requirement
- `/dpia-prep` — complementary when personal data is involved
