---
name: jstack-customer-voice-check
description: Gate customer-facing artifacts through 12-cell Trailblazer eval before distribution.
color: red
tools: Read, Bash, Glob
voice: mixed
cli_support: [claude-code, codex]
license_note: depends on TRAILBLAZER-CORPUS.md calibration (T0)
---

# /customer-voice-check

The voice gate. Every customer-bound artifact (PDF, customer-guide, release note, demo script, email draft) MUST pass this check before it leaves the building. Applies the 12-cell Trailblazer rubric from TRAILBLAZER-TEST.md per paragraph and surfaces violations.

T0 calibration must be complete (`TRAILBLAZER-CALIBRATION.md` ≥90% per cell × ≥10 cells) for this check to be trusted. Pre-calibration runs WARN-only with explicit "uncalibrated" stamp.
...
1. **Preflight.** Verify TRAILBLAZER-CORPUS.md exists + TRAILBLAZER-CALIBRATION.md shows ≥90% per cell × ≥10 cells. If not: surface UNCALIBRATED warning, proceed with WARN-only mode.
...
Calibration status: CALIBRATED (TRAILBLAZER-CALIBRATION.md ≥90% per cell × 11 cells)
Threshold: 85
...
[P3] paragraph 11
   ...
   Issue: hedge phrase "we believe" — see TRAILBLAZER-CORPUS.md Tier 3 traps
   Fix: show, don't say
...
- A FAIL verdict from this skill BLOCKS downstream `/ship` for the customer-bound artifact.
- An UNCALIBRATED stamp surfaces in any downstream reference — `/ship` will refuse to land artifacts gated by an UNCALIBRATED check.
...
```
> /customer-voice-check --input docs/guides/billing.md
Score 88/100, 0 P1, 1 P2, 3 P3. ✓ PASS (fix P2+P3 recommended).
```

**Per-paragraph deep:**
```
> /customer-voice-check --input release-notes-draft.md --per-paragraph --out check-report.md
12 paragraphs evaluated, report at check-report.md.
```

**Strict threshold:**
```
> /customer-voice-check --input pitch-deck.md --threshold 95
Score 88/100 — below threshold (95). ✗ FAIL. Fix violations + retry.
```

## See also

- TRAILBLAZER-TEST.md — the eval prompt this skill applies
- TRAILBLAZER-CORPUS.md — the calibration anchor
- TRAILBLAZER-CALIBRATION.md — current per-cell accuracy
- `/msvoice-rewrite` — rewrite an internal-voice draft to trailblazer
- `/ship` — reads this skill's verdict as a gate
- `/jstack-eval` (Phase 8) — runs this against TRAILBLAZER-CORPUS.md to maintain calibration
