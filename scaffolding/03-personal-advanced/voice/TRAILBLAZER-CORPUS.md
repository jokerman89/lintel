...
**Status:** READY for `/jstack-eval` against `TRAILBLAZER-TEST.md`. T0 unblocked. Phase 3 can start.

---

## Operator workflow (post-population)

1. **Run `/jstack-eval`** (Phase 8 skill — to be written) against this corpus. The eval applies TRAILBLAZER-TEST.md per-paragraph and compares verdict to `verdict_label`.

2. **Record per-cell accuracy in `TRAILBLAZER-CALIBRATION.md`.** Target: ≥90% known-good correctly verdicted + ≥90% known-bad correctly verdicted per cell.
