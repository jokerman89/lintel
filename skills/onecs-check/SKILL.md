...
1. **Read on-demand items.** From `scaffolding/02-sdl/ON-DEMAND-RULES.md` (the 7 items, listed in Phase 6 content).
2. **For each item, run the structured check:**
   ...
   - **Item 4: Sensitive-use case** — does this fit a sensitive-use category per MS RAI? If yes: One RAI Sensitive Uses prep required; trigger `/rais-sensitive-use`.
   - **Item 5: DSB (Data Sharing Board)** — does this share data outside intended audience? If yes: prep DSB submission via `/dsb-submit-draft`.
   - **Item 6: DPIA (Data Protection Impact)** — does this process personal data? If yes: prep DPIA via `/dpia-submit-draft`.
   - **Item 7: Transparency note / disclosure** — does this customer-facing output need a transparency note? If yes: prep via `/rais-transparency-note`.
...
NEEDS_ACTION — demo references "production customer data" without class declaration
   ...
   Follow-up: re-run /onecs-check after sanitization
...
- A NEEDS_ACTION item BLOCKS downstream `/release-ev2` of customer-bearing artifacts.
...
**Pre-demo gate:**
```
> /onecs-check --artifact docs/customer-demo-001.md
...
- `/rais-sensitive-use`, `/dsb-submit-draft`, `/dpia-submit-draft`, `/rais-transparency-note` — Item-specific follow-up skills
- `/release-ev2` — refuses customer-bearing artifacts with open NEEDS_ACTION items
