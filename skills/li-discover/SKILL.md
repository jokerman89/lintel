...
Invoke `/lintel:li-lessons` skill OR inline:
...
- Flag operator: "Skill /lintel:li-<existing> already covers <area>. Extend it or build new?"
- Recommend `/lintel:li-skillify` if creating new vs extending
...
- 8 ADRs (~12k tokens) — `/lintel:li-context-warm-adrs networking`
- Related skills overlap (~5k tokens) — `/lintel:li-context-warm 'skills/li-az-*'`
- Customer Bicep templates (~15k tokens) — `/lintel:li-context-warm '~/Workspace/customer-X/bicep/*'`
...
- /lintel:li-<existing>: <% overlap> — <extend vs new?>
...
**Triggers:**
- `/lintel:li-context-warm` recommendations (operator-driven, not auto)
- `/lintel:li-plan` next (or PLAN within /lintel:li-cycle)
