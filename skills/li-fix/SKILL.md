...
- New feature work — use `/lintel:li-cycle` (full)
- Unclear root cause — use `/lintel:li-investigate` first, then `/lintel:li-fix`
- Customer-deliverable involved — use `/lintel:li-cycle --mode customer-engagement` (voice/brand gates)
...
- If operator hesitates: suggest `/lintel:li-investigate` or `/lintel:li-cycle` instead

### Step 2 — Delegate to /lintel:li-cycle

```bash
/lintel:li-cycle --mode hotfix --from SENSE --to SHIP --skip DEFINE,DISCOVER,PLAN,CAPTURE
```
...
If this fix reveals a pattern worth capturing (lesson for /lintel:li-lessons-promote),
run /lintel:li-capture manually now.
```
...
Inherits from /lintel:li-cycle. Status reflects underlying cycle outcome.
...
Delegates to `/lintel:li-cycle --mode hotfix`. No new behavior beyond that.
