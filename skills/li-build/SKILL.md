...
Skill: /lintel:li-build (task T<N> of plan.md)
[/lintel-context]"
```

NEVER `git add -A`. NEVER commit broken state. Operator can resume from any WIP via `/lintel:li-context-restore`.
...
1. Run full test suite (`/lintel:li-qa` invoked)
...
YES — operator can `/lintel:li-build` with existing plan.md.
...
**Triggers:**
- `/lintel:li-review` next (or REVIEW in /lintel:li-cycle)
...
- **Test suite breaks during task**: revert task, mark BLOCKED, investigate via `/lintel:li-investigate`.
...
`voice: internal`. Code is engineering-internal. Customer-facing artifacts (if any in BUILD) flow through voice gate per `/lintel:li-rais-customer-voice-check`.
