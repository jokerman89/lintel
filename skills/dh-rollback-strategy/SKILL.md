...
deployment_plan=$(find .lintel/state/dh -name "deployment-plan-*.md" -mtime -7 2>/dev/null | sort | tail -1)
...
irreversible_count=$(jq -r '.failure_modes[] | select(.rollback_path == "none" or .rollback_path == "irreversible") | .name' .lintel/state/dh/rollback-mechanics.json 2>/dev/null | wc -l)
...
**Writes:** `.lintel/state/dh/rollback-strategy-<ts>.md`, audit JSONL
