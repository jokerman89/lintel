...
This is the shared method for web-session browse, open, cookies, scrape and
...
`skills/web-session/scripts/chromium.mjs` is a small **single-page Chromium CDP adapter**.
...
`web-session --mode open --check` maps to `checkProvider(executable)` (no launch).
...
PDF inspection requires a separately available, explicitly authorized reader.
No reader is bundled or installed by this workflow. Unavailable inspection remains
unverified; text origins are not complete glyph bounds or PDF raster review.
The recorded reader crash and separately denied native raster-inspection attempt
remain failures, not replaced observations. Historical reader observations do not
establish inspection after the upstream PDF-reader removal.
It is not an A14
