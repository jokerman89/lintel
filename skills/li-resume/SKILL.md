...
- Operator types `/lintel:li-resume`

## When NOT to use

- Brand-new cycle in clean repo (use `/lintel:li-cycle` or `/lintel:li-sense`)
...
If still no state: surface "No prior state found. Run `/lintel:li-cycle` for new work or `/lintel:li-sense` for diagnostic."
...
- If A (next recommended): `/lintel:li-<recommended-phase>`
- If B (restart prior): `/lintel:li-<last-phase>` (re-runs from start)
- If C (specific): `/lintel:li-<chosen-phase>`
- If D (full cycle): `/lintel:li-cycle` (from start, ignoring prior state)
...
- **State file corrupt**: surface, suggest manual reconstruction OR start fresh with `/lintel:li-cycle`
- **No state found**: NOT a failure — gracefully redirect to `/lintel:li-sense` or `/lintel:li-cycle`
