...
Equivalent to:
```
/lintel:li-cycle --from PLAN --to BUILD
```
...
- No APPROVED design doc — use `/lintel:li-cycle` or `/lintel:li-define` first
- Ready to ship — use `/lintel:li-cycle` (full) or compose with `/li-review-and-ship` after
- Hotfix — use `/li-fix` (skips PLAN's heavy ceremony)
- Just plan, no build — use `/lintel:li-plan` standalone
...
If missing: surface, suggest `/lintel:li-define` first OR `/lintel:li-cycle` for full chain.

### Step 2 — Delegate

```bash
/lintel:li-cycle --from PLAN --to BUILD --skip CAPTURE
```
...
To finish cycle later:
  /lintel:li-resume → picks up at REVIEW
  OR
  /lintel:li-review-and-ship → composite for the rest
...
Inherits from /lintel:li-cycle. Most relevant: BUILD DONE/BLOCKED.
...
Delegates to `/lintel:li-cycle --from PLAN --to BUILD`.
