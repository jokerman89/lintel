...
Reads a specific prior session's `/lintel:li-context-save` output and dumps it into current session. Useful when operator knows exactly which session's state to recover.
...
- Don't know which session — use `/lintel:li-context-warm-sessions <N>` instead (loads last N)
- Just want most-recent state — `/lintel:li-resume` handles that
- Loading entire session history — too heavy; use `/lintel:li-context-warm-sessions` with limit
...
candidates=$(find ~/.lintel/sessions/$branch -name "*${session_id}*" -name "*-context-save.md" 2>/dev/null)

if [ -z "$candidates" ]; then
  echo "No session matched '$session_id' on branch '$branch'"
  # List recent sessions
  /lintel:li-context-warm-sessions --list
  exit 1
fi
...
```bash
/lintel:li-context-warm "<full-path>"
```
...
Reads `~/.lintel/sessions/<branch>/`. Delegates to `/lintel:li-context-warm`.

## Anti-patterns

- **Dumping ancient sessions** (>90 days): stale assumptions, surface age warning
- **Dumping multiple sessions back-to-back**: use `/lintel:li-context-warm-sessions` for that pattern
