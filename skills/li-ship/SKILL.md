...
- Operator types `/lintel:li-ship` to ship existing branch
...
- All tests pass (run `/lintel:li-qa` if not already passed in REVIEW)
...
- Invoke `/lintel:li-rais-customer-voice-check` (already done in REVIEW Stage 3, but final verification)
- If new edits since REVIEW: re-run
- Threshold: ≥85% known-good match

If `artifact_kind=customer-deliverable` (PPT/Word/Web):
- Invoke `/lintel:li-generate-ppt` / `-word` / `-web` 4-gate pipeline:
...
Invoke `/lintel:li-provenance-track`:
...
If audience=customer AND AI-system shipped: also invoke `/lintel:li-rais-transparency-note` to draft transparency note for customer.
...
## Provenance
AI-assisted via /lintel:li-cycle v3.5
Phases: DEFINE → PLAN → BUILD → REVIEW → SHIP
...
- `/lintel:li-onebranch-validate` if 1ES pipeline involved
- `/lintel:li-release-ev2` (or `release-deploy-ev2`) for EV2 ring deployments
- `/lintel:li-safe-deploy-ring` for canary strategy
...
**PPT path:**
```bash
# Invoke /lintel:li-generate-ppt with 4-gate pipeline
...
**Word path:**
```bash
# /lintel:li-generate-word with WordTechnicalEditor agent
# Variants: technical / customer-summary / transparency-note
```

**Web path:**
```bash
# /lintel:li-generate-web with WebExperienceCritic agent  
...
- Invoke `/lintel:li-landing-report`
...
YES — operator can `/lintel:li-ship` on existing branch outside full cycle.
...
- **Customer wants to delay deliverable**: SHIP completes commit/PR but skips customer-handoff. Resume customer-handoff later via `/lintel:li-demo-deliverable-gen`.
