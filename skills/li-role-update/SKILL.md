...
- Initial role creation — use `/lintel:li-role-new`
- One-off observation — keep in session notes, only update file if pattern is durable
- Wholesale role rewrite — use `/lintel:li-role-new` to re-scaffold
...
If no role specified and none active: prompt `/lintel:li-roles-list`.
