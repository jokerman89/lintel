---
name: li-context-snapshot
layer: foundation
description: Save current context state to disk — operator-named snapshot for later resume via /lintel:li-context-dump.
color: cyan
...
Saves current session state to `~/.lintel/sessions/<branch>/<datetime>-snapshot-<name>.md`. Different from `/lintel:li-context-save` (which writes canonical end-of-session save) — snapshot is operator-named mid-session preservation.
...
- End of session (use `/lintel:li-context-save` — canonical save)
...
echo "Restore later: /lintel:li-context-dump $(basename $snap_path)"
```
