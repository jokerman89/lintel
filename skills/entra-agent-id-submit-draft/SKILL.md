...
4. **Cross-link.** If agent involves AI: cross-link to `/onerai-submit-draft` and `/rais-sensitive-use`.
5. **Mark DRAFT.**
6. **Report.**
...
## Cross-references
- AI involvement: yes → see /onerai-submit-draft --feature case-intake
- Sensitive use: see /rais-sensitive-use --feature case-intake
...
- Cross-links with `/onerai-submit-draft` for AI agents and `/rais-sensitive-use` for sensitive-use category.
...
- **Sensitive-class data + no Privacy review reference:** WARN — recommend `/dpia-submit-draft`.

## Examples

**User-context agent:**
```
> /entra-agent-id-submit-draft --agent-name case-intake-agent
...
**App-only service:**
```
> /entra-agent-id-submit-draft --agent-name nightly-analytics-aggregator
...
- `/onecs-check` Item 1 — AGT/agent-governance trigger
- `/onerai-submit-draft` — for AI agents
- `/rais-sensitive-use` — for sensitive-use category agents
- `/agt-tier-stamp` — for license/precedence (different concern than Entra ID)
