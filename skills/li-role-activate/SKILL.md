---
name: li-role-activate
layer: foundation
description: Activate a role for the current session — loads role IDENTITY + voice + outcome-lens (LIGHTWEIGHT ~500 tokens). Deep-dive via /lintel:li-role-deep-dive.
color: cyan
...
Does NOT load full role file. For deep knowledge access on-demand, use `/lintel:li-role-deep-dive <role-id>`.
...
- When operator wants role's FULL knowledge — use `/lintel:li-role-deep-dive` instead
...
Deep-dive available: /lintel:li-role-deep-dive <role-id>
Rotate role: /lintel:li-role-rotate <other-id>
Deactivate: /lintel:li-role-deactivate
```
...
- **Role file not found**: list available via `/lintel:li-roles-list`, prompt for correct ID
...
`voice: internal`. The role's voice_tier propagates to downstream skills. If role.voice_tier=trailblazer, customer-facing skills (e.g., `/lintel:li-exec-brief`) auto-apply Trailblazer voice gate.
