...
- First role activation — use `/lintel:li-role-activate` directly
- Deactivating without replacement — use `/lintel:li-role-deactivate`
...
If no current role active: surface info, suggest `/lintel:li-role-activate` directly.
...
Delegate to `/lintel:li-role-activate <new-role>`. Loads lightweight context for new role.
...
If you need deep-dive on new role: /lintel:li-role-deep-dive <new-role>
```
...
Delegates to `/lintel:li-role-activate` after deactivating current. Reads + writes `~/.lintel/profile.yaml`.
