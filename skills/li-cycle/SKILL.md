...
- Full: `/lintel:li-cycle` → SENSE → DEFINE → DISCOVER → PLAN → BUILD → REVIEW → SHIP → CAPTURE
- Mode preset: `/lintel:li-cycle --mode hotfix` → runs preset's phase-subset
- Custom: `/lintel:li-cycle --from <phase> --to <phase> --skip <phases>` → operator-specified subset
- Auto: `/lintel:li-cycle --mode auto` → SENSE detects intent + recommends mode

Each phase is its own skill (`/lintel:li-sense`, `/lintel:li-define`, etc.). CYCLE chains them with gates between, propagates context, handles pause-points.

## When to use

- Full feature/cycle work where operator wants the structured path
- When operator types `/lintel:li-cycle` (cold start)
- When `/lintel:li-resume` decides to re-orchestrate from a phase

## When NOT to use

- Single skill invocation (just call the phase-skill directly: `/lintel:li-review`)
...
If operator pauses: state saved to .lintel/state/00-state.md with `cycle_paused: true`. Resume via `/lintel:li-resume`.
...
YES — `/lintel:li-cycle --from <phase>` enters at specified phase.
...
**Triggers:**
- Each phase-skill in sequence: `/lintel:li-sense`, `/lintel:li-define`, etc.
...
- **Nesting cycles** — one cycle at a time, no recursive /lintel:li-cycle from within
