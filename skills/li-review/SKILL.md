...
- After BUILD completes (auto-invoked in /lintel:li-cycle)
- Standalone on existing diff/PR (operator types `/lintel:li-review`)
...
- intent=docs-only (lighter review path — invoke `/lintel:li-docs-review` if exists, or skip)
...
**`/lintel:li-caip-audit`** (if WorkProfile=on):
...
**`/lintel:li-onecs-check`** (if WorkProfile=on AND audience=customer):
- 1CS-specific compliance gates
- Customer-deliverable readiness

**`/lintel:li-rais-customer-voice-check`** (if voice_tier=trailblazer AND audience=customer):
...
**`/lintel:li-agt-tier-stamp`** (if BUILD includes agentic system):
...
- `/lintel:li-review --diff origin/main..HEAD` — review against main
- `/lintel:li-review --pr 42` — review specific PR
