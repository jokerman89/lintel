...
Invoke `/lintel:li-learn` (or inline):
...
- "Always run /lintel:li-az-tldr first when planning Azure work — saved 90 min on this engagement"
...
If yes: invoke `/lintel:li-lessons-promote`.
...
If yes: invoke `/lintel:li-adr-new "<decision title>"`:
...
2. Read plan.md
3. Run /lintel:li-cycle --from BUILD (skip DEFINE/PLAN, they're done)
4. Apply two-stage review per task
5. Run /lintel:li-qa final
6. Ship per /lintel:li-ship
...
- /lintel:li-resume picks up next cycle from here
```
...
- `/lintel:li-resume` available for next cycle
