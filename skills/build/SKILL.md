...
The established task-by-task workflow below remains the default. Enter `/li:swarm run` only when a
...
2. Treat `frontier.dispatch_task_ids` as an evidence/topology candidate from the earliest incomplete
   wave, not authoritative dependency resolution. Re-read each candidate task's complete mapped card
   and brief. Before every worker handoff, inspect every card prerequisite and verify completion
...
6. After a valid worker report, a distinct reviewer writes only that lane's review artifact: Stage 1
   checks the complete card/spec, then Stage 2 checks quality. Both must PASS. An unavailable
   independent reviewer leaves the lane open; it is never converted to fabricated PASS evidence.
...
The detailed per-task rules below remain the worker/reviewer discipline for each swarm lane; only
candidate-wave scheduling and isolation change; mapped card prerequisites remain authoritative.
