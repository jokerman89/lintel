...
- After a Lintel update (`bash install/install.sh` re-run) — confirm nothing broke
- When a skill or agent behaves oddly — confirm it's installed at the right path with the right frontmatter
- Teammate reports "Lintel isn't working" — first triage step
...
1. **Install manifest:** read `~/.claude-scaffolding/INSTALL-MANIFEST.json`. Confirm fields present (installed_at, repo_commit, layers_enabled, upstream_sha_per_source, host_platform, host_user, lintel_version).
...
**Issues (full):**
```
Lintel v1.0.0 health: ⚠ 3 issues, 1 warning
...
1. Re-run install.sh to refresh skill frontmatter for li-foo and li-bar
...
- **`~/.claude-scaffolding/` doesn't exist:** report "Lintel not installed. Run install.sh."
...
```
> /health --fast
Lintel v1.0.0 health: ✓ all checks pass (5s)
```
