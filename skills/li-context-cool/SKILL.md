...
For real budget reduction: operator can `/lintel:li-context-snapshot` then restart session, then `/lintel:li-context-dump` selectively.
...
True budget reduction requires session restart:
  1. /lintel:li-context-snapshot
  2. Restart session
  3. /lintel:li-context-dump <snapshot>
  4. /lintel:li-context-warm (only what's still needed)
```
