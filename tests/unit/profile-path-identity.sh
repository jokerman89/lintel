#!/usr/bin/env bash
# component: profile-path-identity-runner
# implements: ADR-0029
# intent: docs/concepts/pack-resolver.md
# constraints: synthetic fixtures only; no private paths or network UNC access
...
if command -v python >/dev/null 2>&1 && python -c 'import sys; assert sys.version_info >= (3, 9)' 2>/dev/null; then
  ...
  echo 'FAIL: Python 3.9+ is required for profile path verification' >&2
  exit 1
fi
export PYTHONDONTWRITEBYTECODE=1
"$PYTHON" "$ROOT/tests/unit/profile-path-identity.py" --root "$ROOT" "$@"
