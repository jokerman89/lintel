...
# constraints: disposable owned files only
...
if command -v python3 >/dev/null && python3 -c 'import sys; assert sys.version_info >= (3, 10)' 2>/dev/null; then
  ...
  echo 'ERROR: Python 3.10+ is required for snapshot-ownership tests.' >&2
  exit 1
fi
PYTHONDONTWRITEBYTECODE=1 "$python" "$root/tests/unit/snapshot-ownership.py"
