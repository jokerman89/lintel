...
- [docs/audit/lintel-uniformity-REMEDIATION-REVIEW.md](lintel-uniformity-REMEDIATION-REVIEW.md) — precedent already ruled: leverage-first, themed PRs, no contract-before-consumer. This plan inherits it.
...
- **Design-doc Swedish** (`docs/design/*`, `docs/audit/*`) — lower portability priority than the shipped surface. The tripwire (T2) scopes to skills/agents/hooks first; design docs are a follow-up so the guard doesn't block on a verbatim operator quote.
...
- [ ] **T1 (P1, human ~4h / CC ~30min)** — i18n — English sweep of the v3.5–v3.7 cohort
...
- [ ] **T2 (P1, human ~1h / CC ~15min)** — ci — Swedish tripwire CI guard [D4]
...
- [ ] **T3 (P2, human ~2h / CC ~20min)** — release — identity single-source + sync test [D2]
...
- [ ] **T4 (P2, human ~1h / CC ~10min)** — docs — parameterize hard-coded paths + operator examples
...
- [ ] **T5 (P1, human ~30min / CC ~5min)** — wiki — regenerate counts + showcase
...
- [ ] **T6 (P2, human ~2h / CC ~20min)** — docs — reconcile the fossils
...
- [ ] **T8 (P1, human ~3h / CC ~25min)** — hooks — shared stdin adapter [A1, dual-mode]
  ...
  - Verify: T9.
- [ ] **T9 (P1 CRITICAL regression, human ~1h / CC ~15min)** — hooks — adapter shape test
  - stdin JSON → correct field extracted; empty stdin → `$1` fallback; malformed JSON → graceful, no crash.
- [ ] **T10 (P1, human ~1h / CC ~15min)** — hooks — PreCommit remap [D1]
...
- [ ] **T11 (P1, human ~2h / CC ~20min)** — cost — wire the 500k cap + gate BUILD review
...
- [ ] **T12 (P2, human ~45min / CC ~10min)** — cost — cap-fires test
  - trio + warming context > 500k → cap fires at the handoff.
- [ ] **T13 (P1, human ~1h / CC ~15min)** — routing — `deploy` as a distinct intent [D3]
...
- [ ] **T14 (P1 CRITICAL regression, human ~45min / CC ~10min)** — routing — intent regression test
  - `"deploy X"`→`deploy`; `"ship X"`→`ship`; smoking-gun `"deploy a website to azure"` still escalates.
- [ ] **T15 (P2, human ~3h / CC ~25min)** — dispatch — wire the orphans
...
- [ ] **T16 (P2 regression, human ~30min / CC ~5min)** — capture — fix `_audit.sh` source path [Q4]
  - `capture/SKILL.md:61` `$(dirname "$0")`→`$LINTEL_REPO_ROOT`. Verify: calibration write actually lands (it silently no-ops today).
- [ ] **T17 (P2, human ~1h / CC ~10min)** — skills — fix lessons-surface invocation + root guard
...
- [ ] **T18 (P2, human ~2h / CC ~20min)** — hooks — extract `_patterns.sh`
...
- [ ] **T19 (P3, human ~3h / CC ~30min)** — cleanup — the rest of the consolidations
