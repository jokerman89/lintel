...
You will work in **eight cohorts**, in this order. After each cohort, checkpoint findings to `docs/audit/lintel-uniformity-findings-<cohort>.md`, then move to the next.
...
**Per-cohort:** `docs/audit/lintel-uniformity-findings-<cohort-name>.md` with one finding record per component plus a cohort summary.

**Cross-cutting:** `docs/audit/lintel-uniformity-cross-<X-number>.md` for X1 through X5.

**Master summary:** `docs/audit/lintel-uniformity-MASTER.md` — top 20 highest-leverage findings, ranked. Each entry: which dimension, which components affected, proposed uplift, why this specifically, estimated effort, dependencies on other findings.

**Voting register:** `docs/audit/lintel-uniformity-VOTE.md` — every finding marked `operator_decision_required: yes`, listed for operator to vote on. Format: finding ID, one-line summary, three proposed approaches (chosen + two alternatives), recommendation. Operator votes by marking choice.
