...
Before committing to full calibration ordering, verify `/li:eval` actually produces parseable verdicts.
...
2. **Invoke `/li:eval --cells R1,CAIP --paragraphs R1-GOOD-001,R1-BAD-001,CAIP-GOOD-001 --dry-run`**.
...
```
/li:eval --corpus scaffolding/03-personal-advanced/voice/OurVoice-corpus.md \
...
4. Re-run eval scoped to changed cells: `/li:eval --cells <changed>`
...
- [ ] Confirm `/li:eval` produces parseable verdicts
...
| Issue | Mitigation |
|-------|------------|
| Smoke test fails — output unparseable | Fix `/li:eval` skill before continuing. Likely YAML escape issue or rubric ambiguity. |
...
- `/li:eval` skill — eval orchestrator
