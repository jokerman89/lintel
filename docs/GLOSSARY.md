...
| **workflow_root** | A skill that owns a job and can spawn sub-work — the top of a hand-off tree (`/li:cycle`, `/li:plan` standalone). Marked `workflow_root: true` in frontmatter; hand-offs out of it pass through the Brief Forge envelope (`lib/envelope-schema.yaml`) — envelope construction is dormant by decision (ADR-0008): opt-in, not auto-armed. |
