...
- **Public-tree move — NOT done (reverted).** The plan was to untrack internal-only docs. An
  adversarial review of the diff found this was unsafe: `.claude/engineering/audits/uniformity-matrix.md` is a LIVE
  ...
  `lib/state.sh` + `lib/auto-decide.sh` (the comment contract). Untracking `.claude/engineering/audits/` would 404
  ...
  moved out of `.claude/engineering/audits/` + the `# intent:` headers + shipped-doc citations repointed first.
...
motivated deviation. Report: `.claude/engineering/compat-audits/2026-06-18-*.md`.
