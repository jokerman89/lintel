...
**Restore point:** `.claude/engineering/audits/.restore-v4.0-phase1-meta-infra-spine-20260529-183430.md`
...
```
 ...
 (mechanical: presence + per-kind floor)      of the 8 that already exist)
        ...
        ▼
 .claude/engineering/audits/uniformity-matrix.md            ← regenerated + idempotent like CATALOG.md;
...
1. `.claude/engineering/audits/uniformity-contract.md` — the per-(kind × dimension) target table + the `uniformity:` frontmatter schema (which of D1–D14 each kind must declare, which are N/A-with-reason).
2. `tests/shape/uniformity-coverage.sh` — Gate-M3 test #9: per-kind floor check, exits ≠0 only on below-floor.
3. `.claude/engineering/audits/uniformity-matrix.md` — regenerated, idempotent (sibling of `catalog-regenerates-clean.sh`); component × dimension, adoption % per unbuilt feature.
