...
Cross-session working state (not durable rules — that's [[lessons.md]]; not persona frames — that's
[[personas.md]]). Surface at session-start so the operator can see where the work paused.

> Format per entry: short title, then `Status:`, then `What's pending:`. Update at session-end
> or at major checkpoints. Stale entries (>30 days) should be cleaned up.
...
is **108/113**, not full release readiness. Open: A15.1–.4 (denied routes) and A23.4 (strict CI,
...
- **Release (B8):** truthful CHANGELOG 5.3.0, migration date reconcile, M1 artifact
  (.claude/engineering/evolution/2026-06-13-launch-readiness.md). CODEOWNERS de-branded.
...
**What's pending:**
- ~~Phase 2: --from-pipeline support~~ ✅ SHIPPED (PR #17)
- ~~Phase 3: generate-style-learn skill~~ ✅ SHIPPED (PR #19)
...
**What's pending:**
- ~~Phase A1: foundation core~~ ✅ SHIPPED (PR #22)
- ~~Phase A2: extension + canonical pattern~~ ✅ SHIPPED (PR #23)
- ~~Phase B: generate-web --from-frontend-design + new generate-app skill (M-2)~~ ✅ SHIPPED (PR #24)
- ~~Phase C: frontend-design-surface hook + vault loop closure~~ ✅ SHIPPED (PR #25)
- v3.7.0-dev tag ✅ pushed 2026-05-29
- **Phase D** (operator-only) — real-engagement dogfood + L-004 canonical-pattern re-evaluation pending
...
**Status:** active (Cohort 4 operator-only work remaining)
...
- **Cohort 4a (alias-mechanism design pass)** — PR #14 open (intentional — waiting on WS-4a/b)
- **Cohort 4 implementation** — depends on operator working sessions WS-4a (gstack-collisions) + WS-4b (orphan-names)

**Last touched:** 2026-05-28 (~95% v3.6 backlog completed)

---

## reviewer-concerns — Open from earlier PRs (M-1 tracking per v3.6 backlog)
...
1. ~~**Voice-gate terminology mismatch**~~ ✅ CLOSED. Audit confirmed the generate-* family calls the pack's customer-voice-check skill consistently (no references to the design doc's older critic-agent name in skills/). Terminology unified — the design doc had the stale name.
2. ~~**4-gate explicit home**~~ ✅ CLOSED. `skills/generate/agent-mapping.yaml` has `voice_gate_owner: orchestrator` + `format_gate_owner: format-builder` explicit. Validated at the v3.5 Phase 2 merge (PR #17).
...
4. ~~**Voice-blocklist-customer-share-gate interaction**~~ ✅ CLOSED. Full interaction chain documented in `skills/generate/SKILL.md` "Voice-blocklist ↔ customer-share-gate interaction" section: --customer-share → the pack's customer-facing draft voice tier → blocklist enforced via the pack's voice corpus → the pack's customer-voice-check verifies + compliance-gate aggregates.

### PR #9 (lintel-v3.6-backlog-sequencing) — 7 of 7 concerns RESOLVED (2026-05-29 sweep)

MAJORs (#1, #2): coverage matrix shipped inline; 4.1 alias-mechanism CLOSED via PR #14 (design) + PR #27 (implementation 2026-05-29).
...
Eng-review run 2026-05-28. v3.7 phases A1+A2+B+C shipped in PR #22-#25, all merged 2026-05-29. Status update for 9 concerns:

**MAJORs — all 6 RESOLVED:**
1. ~~M-1 design-spec.json schema collision~~ ✅ RESOLVED in PR #21 (filename → `frontend-design-spec.json`) + PR #24 (generate-web reader)
2. ~~M-2 frontend-app-scaffold boundary violation~~ ✅ RESOLVED in PR #21 (renamed to generate-app) + PR #24 (skill shipped in the generate-* family)
3. ~~M-3 Phase A monolithic-PR risk~~ ✅ RESOLVED in PR #21 (split into A1 + A2)
4. ~~M-4 sequential sub-skill chain 3× latency~~ ✅ RESOLVED in PR #22 (orchestrator Workflow Step 2-4 parallel-dispatch documented)
5. ~~M-5 schema-versioning missing~~ ✅ RESOLVED in PR #22/#23 (`schema_version: 1` on all contract JSON)
6. ~~M-6 roundtrip integration test deferred~~ ✅ RESOLVED in PR #22 (`tests/integration/frontend-design-roundtrip.sh`)

**MINORs — 2 of 3 RESOLVED, 1 deferred:**
7. ~~m-1 FrontendArchitect ↔ FrontendBuilder non-overlap~~ ✅ RESOLVED in PR #22 (explicit non-overlap section)
8. **m-2 SKILL.md DRY pattern** — ⏳ DEFERRED. 9 frontend-* + generate-* skills now exist; pattern-anatomy doc not yet authored. Low-priority — drift signal-to-noise stays low as long as the count is manageable. Address when next family adds.
9. ~~m-3 --overwrite flag inheritance~~ ✅ RESOLVED in PR #23 (frontend-style-extract documents flag)

**Additional concern surfaced post-shipment (#7 perf-budget):** vault-lookup latency budget documented in PR #25 frontend-design-surface hook (<200ms MVP target for vault of 1-3 patterns, scaling-index deferred until the vault exceeds 10).

**Status:** PR #21 reviewer-concerns 7-of-9 closed via implementation. Remaining m-2 carries over to the next family-design occasion. Entry can close once m-2 is addressed or the operator dismisses it as YAGNI.

**Last touched:** 2026-05-29 (v3.7.0-dev milestone)

---

## operator-only-remaining — Items that need the operator, beyond what can be automated
